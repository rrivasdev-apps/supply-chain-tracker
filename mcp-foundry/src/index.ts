...
const PROJECT_ROOT =
  process.env.FOUNDRY_PROJECT_ROOT ?? path.resolve(__dirname, "../..");
const SC_DIR =
  process.env.FOUNDRY_SC_DIR ?? path.join(PROJECT_ROOT, "sc");
...
function run(cmd: string, cwd: string = SC_DIR): string {
  try {
    ...
    const out =
      err.stdout instanceof Buffer
        ? err.stdout.toString()
        : (err.stdout as string | undefined) ??
...
const TOOLS: Tool[] = [
  ...
  },
  {
    name: "forge_test",
    description:
      "Run the Foundry test suite. Supports filtering by test or contract name.",
    inputSchema: {
      type: "object",
      properties: {
        match_test: {
  ...
  },
  {
    name: "forge_deploy",
    description:
      "Deploy the SupplyChain contract using Deploy.s.sol via `forge script --broadcast`.",
    inputSchema: {
      type: "object",
      properties: {
        private_key: {
        ...
        script: {
          type: "string",
          description:
            "Foundry script file to run (default: script/Deploy.s.sol)",
  ...
  },
  {
    name: "cast_call",
    description:
      "Call a read-only contract function with `cast call`. Returns the raw decoded output.",
    inputSchema: {
      type: "object",
      properties: {
        address: {
  ...
  },
  {
    name: "cast_send",
    description:
      "Send a state-changing transaction to a contract with `cast send`.",
    inputSchema: {
      type: "object",
      properties: {
        address: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      // ── forge ──────────────────────────────────────────────────────────────
      case "forge_build": {
        const extra = (a.extra_args as string | undefined) ?? "";
      ...
      case "forge_test": {
        let cmd = "forge test";
        if (a.match_test) cmd += ` --match-test "${a.match_test}"`;
        if (a.match_contract) cmd += ` --match-contract "${a.match_contract}"`;
        const v = (a.verbosity as number | undefined) ?? 3;
        cmd += ` -${"v".repeat(Math.min(Math.max(v, 1), 5))}`;
        if (a.gas_report) cmd += " --gas-report";
        return text(run(cmd));
      }
      ...
      case "forge_deploy": {
        const pk = a.private_key as string;
        const rpc = (a.rpc_url as string | undefined) ?? "http://localhost:8545";
        const script = (a.script as string | undefined) ?? "script/Deploy.s.sol";
        const broadcast = (a.broadcast as boolean | undefined) !== false;
      ...
      case "anvil_start": {
        ...
        const port = (a.port as number | undefined) ?? 8545;
        const chainId = (a.chain_id as number | undefined) ?? 31337;
        const anvilArgs: string[] = [
          "--port", String(port),
          "--chain-id", String(chainId),
        ];
        if (a.block_time) anvilArgs.push("--block-time", String(a.block_time));
        if (a.accounts) anvilArgs.push("--accounts", String(a.accounts));
        if (a.silent !== false) anvilArgs.push("--silent");
        ...
        await new Promise<void>((resolve, reject) => {
          const tid = setTimeout(
            () => reject(new Error("Anvil did not start within 10 s")),
            10_000
          );
        ...
        return text(
          `Anvil started — port ${port}, chainId ${chainId}, PID ${anvilProcess.pid}`
        );
      }

      case "anvil_stop": {
        if (!anvilProcess) return text("No Anvil process is running.");
      ...
      case "cast_call": {
        const rpc = (a.rpc_url as string | undefined) ?? "http://localhost:8545";
        const callArgs = ((a.args as string[] | undefined) ?? [])
      ...
      case "cast_send": {
        const rpc = (a.rpc_url as string | undefined) ?? "http://localhost:8545";
        const sendArgs = ((a.args as string[] | undefined) ?? [])
          .map((x) => JSON.stringify(x))
          .join(" ");
        const value = a.value ? `--value ${a.value}` : "";
        const cmd =
          `cast send ${a.address} "${a.function_sig}" ${sendArgs} ` +
      ...
      case "cast_balance": {
        const rpc = (a.rpc_url as string | undefined) ?? "http://localhost:8545";
        const out = run(
          `cast balance ${a.address} --ether --rpc-url ${rpc}`,
          PROJECT_ROOT
        );
      ...
      case "cast_receipt": {
        const rpc = (a.rpc_url as string | undefined) ?? "http://localhost:8545";
        return text(
          run(`cast receipt ${a.tx_hash} --rpc-url ${rpc}`, PROJECT_ROOT)
        );
      }
