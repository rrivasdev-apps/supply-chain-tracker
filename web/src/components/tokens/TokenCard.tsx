...
export function TokenCard({ token, onTransfer, onBurn, onEdit, showActions = true }: TokenCardProps) {
  ...
  // Carga el conteo de redenciones solo para productos del usuario conectado
  useEffect(() => {
    if (isRawMaterial || !contract || !account) return
    getRedemptions(contract, token.id, account)
      .then((r) => setRedemptionCount(r.length))
      .catch(() => setRedemptionCount(0))
  }, [contract, token.id, isRawMaterial, account])

  const hasRedemptions = (redemptionCount ?? 0) > 0
  const isClickable = !isRawMaterial && hasRedemptions && !onEdit

  const handleCardClick = () => {
    if (isClickable) setHistoryOpen(true)
    else if (onEdit && !token.burned) onEdit(token)
  }
  ...
  return (
    <>
      <Card
        className={[
          token.burned ? "opacity-50" : "",
          isClickable ? "cursor-pointer hover:border-primary transition-colors" : "",
          onEdit && !token.burned ? "cursor-pointer hover:border-primary transition-colors" : "",
        ].filter(Boolean).join(" ")}
              ...
              <div className="flex gap-2 shrink-0 ml-auto items-center">
                {onTransfer && token.balance > 0n && (
                  isRawMaterial && !token.certified ? (
                    ...
                    <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); onTransfer(token) }}>
                      Transferir
                    </Button>
                  )
                )}
                {onBurn && (
                  <Button size="sm" variant="destructive" onClick={(e) => { e.stopPropagation(); onBurn(token) }}>
                    Redimir
                  </Button>
                )}
              </div>
